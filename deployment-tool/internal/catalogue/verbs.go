// Package catalogue is the SoT for eip CLI verbs (id / help titles).
//
...
// Verbs returns the operator command catalogue in menu order.
...
func Verbs() []Verb {
	return []Verb{
		...
		{ID: "secrets", Title: "Secrets", Short: "Sync .env secrets to Swarm and rematerialise mounts"},
		{ID: "rebuild", Title: "Rebuild", Short: "Bake local images and rematerialise (roll only when digests change)"},
