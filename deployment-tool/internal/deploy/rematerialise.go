...
// Rematerialise re-expands and stack-deploys the full stack (data + app + obs).
...
func Rematerialise(ctx context.Context, src Source) error {
	if src != SourceLive && src != SourceDev {
		return fmt.Errorf("rematerialise: source must be live or dev (got %q)", src)
	}
	...
	return stackRedeploy(ctx, home, src, cfg, expandEnv, "Rematerialise")
}

// Rebuild bakes local app images then rematerialises the full stack (dev source).
