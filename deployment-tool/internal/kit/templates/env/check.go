...
// rejectedPlaceholders are non-empty values that must not pass ensure / Ready
// (example-domain / sample SSO strings).
var rejectedPlaceholders = map[string]string{
...
// empty or rejected placeholder values fail this check.
func CheckUsable(home string) error {
	...
	for _, f := range EnvFields() {
		...
		if want, ok := rejectedPlaceholders[f.Key]; ok && v == want {
