# Local Swarm app images (#35). Used by eip dev / eip rebuild (deployment-tool/internal/images).
...
# eip CLI version (host binary ldflag). Source module folder is deployment-tool/.
# Swarm stack/networks stay "eip". Command prefix / binary: eip / eip.exe.
# Target "deployment-tool" (container image) is legacy/deferred — product path is host
# via deployment-tool-windows / deployment-tool-linux (or scripts/deployment-tool/build-host.*).
variable "EIP_CLI_VERSION" {
  default = "0.0.0-dev"
}

variable "ADMINTOOL_IMAGE" {
  default = "ghcr.io/darcy561/eve-industry-planner-deployment-tool"
}
...
target "deployment-tool" {
  context    = "./deployment-tool"
  dockerfile = "Dockerfile"
  tags = [
    "eve-industry-planner-deployment-tool:${EIP_CLI_VERSION}",
...
# Host binaries → repo root (eip.exe / eip). Prefer: .\scripts\deployment-tool\build-host.ps1
target "deployment-tool-windows" {
  context    = "./deployment-tool"
  dockerfile = "Dockerfile"
  target     = "host-windows"
