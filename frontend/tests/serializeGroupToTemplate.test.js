...
import {
  ...
  expectedTotalFromSetups,
  hasParentCycle,
} from "../src/Functions/GroupTemplates/serializeGroupToTemplatePayload";
...
function minimalJob(overrides = {}) {
  ...
  return {
    ...
    build: {
      ...
      products: {
        totalQuantity:
          overrides.totalQuantity !== undefined
...
describe("serializeGroupToTemplatePayload", () => {
  it("serializes a single job", () => {
    const j = minimalJob({ jobID: "job-1" });
    const out = serializeGroupToTemplatePayload({
      groupID: "group-1",
      jobs: [j],
      name: "One",
      description: "d",
  ...
  it("throws when setup total does not match persisted total", () => {
    const j = minimalJob({ totalQuantity: 99 });
    expect(() =>
      serializeGroupToTemplatePayload({
        groupID: "g",
        jobs: [j],
...
describe("expectedTotalFromSetups", () => {
  it("sums runs with itemsProducedPerRun", () => {
    const job = minimalJob({});
    expect(expectedTotalFromSetups(job)).toBe(5);
  });
});
