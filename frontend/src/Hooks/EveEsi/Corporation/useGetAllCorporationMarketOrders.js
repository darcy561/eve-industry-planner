...
/**
 * Utility function to create error object for corporation market orders queries.
 * 
...
/**
 * Utility function to create loading object for corporation market orders queries.
 * 
...
/**
 * Utility function to create success object for corporation market orders queries.
 * 
...
/**
 * Retrieves cached corporation market orders data from React Query cache for all users.
 * 
 * This function provides access to cached corporation market orders data without triggering new queries:
 * - Fetches market orders for all user corporations
 * - Checks loading states for all corporation market order queries
 ...
 * 2. Checks query states for all corporation market order queries
 ...
 * 
 * @example
 * const cachedOrders = getAllCachedCorporationMarketOrders(queryClient);
 * if (!cachedOrders.isLoading && !cachedOrders.isError) {
 *   Object.keys(cachedOrders.data).forEach(corpId => {
 *     console.log(`Corporation ${corpId}: ${cachedOrders.data[corpId].length} active orders`);
 *   });
 * }
 */
...
/**
 * Custom hook that fetches corporation market orders for all user corporations.
 * 
 * This hook provides comprehensive corporation market orders data fetching:
 * - Fetches market orders for all user corporations in parallel
 ...
 * 1. Gets all user character hashes from the store
 * 2. Creates queries for all corporation market order data
 ...
 * 
 * @example
 * function CorporationMarketOrdersManager() {
 *   const { data: ordersByCorporation, isLoading, isError, error } = useGetAllCorporationMarketOrders();
 * 
 ...
 *         <div key={corpId}>
 *           Corporation {corpId}: {ordersByCorporation[corpId].length} active orders
 *         </div>
 *       ))}
 *     </div>
 *   );
