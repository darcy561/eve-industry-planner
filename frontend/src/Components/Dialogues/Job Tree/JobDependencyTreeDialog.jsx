...
import { JOB_DEPENDENCY_TREE_DIALOG_EVENT } from "../../../Events/jobDependencyTreeDialogEvents";
...
/**
 * Global dialog: interactive job dependency tree ({@link JobDependencyTreeFlow}).
 * - `openJobDependencyTreeDialog` — pass job ids, group, highlights, etc.
 * - `openJobLinkTreeFromEditPage` — pass only the edited job id and route search; resolution lives here.
 */
export default function JobDependencyTreeDialog() {
  const [messageData, , resetDialog] = useDialogEventState(
    JOB_DEPENDENCY_TREE_DIALOG_EVENT,
    defaultState
  );
  ...
  useEffect(() => {
    ...
    trackAppEvent(AppEvent.VIEW_JOB_TREE_DIALOG);
  ...
  const onJobDoubleClick = useCallback(
    ...
    [navigate, groupForEdit, editPageView, resetDialog]
  );
  ...
  const emptyMessage = messageData.fromEditContext
    ...
    : "No jobs to show. Pass a group and/or job ids when opening the dialog.";
  ...
  return (
    <ContentDialog
      ...
      componentName="JobDependencyTreeDialog"
      maxWidth="xl"
      fullWidth
