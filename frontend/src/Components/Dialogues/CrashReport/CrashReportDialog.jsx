...
/**
 * Full dialog + form; mounted only when a crash session is active.
...
function CrashReportSession({ eventId, hint, onDismiss }) {
  ...
  return (
    <ContentDialog
      ...
      componentName="CrashReportDialog"
      maxWidth="sm"
      fullWidth
      disableRestoreFocus
      slotProps={{
      ...
      dialogActionsProps={{ sx: { px: 3, pb: 2 } }}
      actions={
        <>
 ...
 * Listens for {@link openSentryCrashReportDialog} from `crashReportEvents` and submits via {@link captureFeedback} + `associatedEventId`.
 */
