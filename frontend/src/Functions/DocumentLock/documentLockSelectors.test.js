...
describe("documentLockSelectors", () => {
  it("selectScopedDocumentLock merges defaults for unknown scope", () => {
    ...
    const merged = selectScopedDocumentLock(s, "user_job_documents", "j1");
  ...
  it("selectDocumentLockReadOnly reads merged readOnly", () => {
    ...
    expect(selectDocumentLockReadOnly(s, "user_job_documents", "j1")).toBe(true);
  });

  it("filterUnlockedDocumentIDs drops read-only ids", () => {
    const k1 = docLockScopeKey("user_job_documents", "a");
    const k2 = docLockScopeKey("user_job_documents", "b");
    const s = stateWithScopes({
      [k1]: { readOnly: true },
      [k2]: { readOnly: false },
    });
    expect(
      filterUnlockedDocumentIDs(s, "user_job_documents", ["a", "b", "c"])
    ).toEqual(["b", "c"]);
  });

  it("filterUnlockedDocumentIDs returns empty for bad inputs", () => {
    expect(filterUnlockedDocumentIDs({}, "", ["a"])).toEqual([]);
    expect(filterUnlockedDocumentIDs(stateWithScopes({}), "user_job_documents", null)).toEqual(
      []
    );
  });
});
