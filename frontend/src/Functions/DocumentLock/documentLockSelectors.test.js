...
describe("documentLockSelectors", () => {
  it("selectScopedDocumentLock merges defaults for unknown scope", () => {
    ...
    const merged = selectScopedDocumentLock(s, "account_job_documents", "j1");
  ...
  it("selectDocumentLockReadOnly reads merged readOnly", () => {
    ...
    expect(selectDocumentLockReadOnly(s, "account_job_documents", "j1")).toBe(true);
  });

  it("filterUnlockedDocumentIDs drops read-only ids", () => {
    const k1 = docLockScopeKey("account_job_documents", "a");
    const k2 = docLockScopeKey("account_job_documents", "b");
    const s = stateWithScopes({
      [k1]: { readOnly: true },
      [k2]: { readOnly: false },
    });
    expect(
      filterUnlockedDocumentIDs(s, "account_job_documents", ["a", "b", "c"])
    ).toEqual(["b", "c"]);
  });

  it("filterUnlockedDocumentIDs returns empty for bad inputs", () => {
    expect(filterUnlockedDocumentIDs({}, "", ["a"])).toEqual([]);
    expect(filterUnlockedDocumentIDs(stateWithScopes({}), "account_job_documents", null)).toEqual(
      []
    );
  });
});
