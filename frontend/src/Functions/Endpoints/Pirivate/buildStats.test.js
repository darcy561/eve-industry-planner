...
const { default: getBuildStatsByTypeID } = await import("./buildStats.js");
...
describe("getBuildStatsByTypeID", () => {
  ...
  it("unwraps the single row from the items list", async () => {
    ...
    const got = await getBuildStatsByTypeID(34);
  ...
  it("requests the totals route with the type as a query parameter", async () => {
    ...
    await getBuildStatsByTypeID(34);
  ...
  it("returns a zeroed aggregate when the account has never built the type", async () => {
    ...
    const got = await getBuildStatsByTypeID(99);
