/**
 * Coalesces rapid `account_job_documents` WS deliveries into fewer Zustand updates.
 */
...
function flush() {
  ...
  if (pendingDeletes.size > 0) {
    ...
    const now = Date.now();
    rs.setCursorMsBatch(
      ids.map((jobID) => [`account_job_documents.${jobID}`, now])
    );
  }
  ...
  if (pendingUpserts.size > 0) {
    ...
    for (const [jobID, doc] of entries) {
      ...
      if (ms != null) {
        cursorPairs.push([`account_job_documents.${jobID}`, ms]);
      }
    }
