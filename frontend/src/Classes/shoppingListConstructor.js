...
class ShoppingList {
    ...
    calculateTotalValue(alternativePriceLocation = {}) {
        ...
        this.items.forEach((item) => {
            if (!item.isVisible || !item.includeWhenCopying) return;
            this.totalValue += item.quantityToPurchase * useUsersStore.getState().worldData.actions.findMarketData(item.typeID, alternativePriceLocation)[defaultMarket][defaultOrders];
        });
    }
