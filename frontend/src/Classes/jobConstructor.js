...
class Job {
   ...
   * @param {number} [itemJson.buildVer] - Build version for compatibility
   ...
   * @param {Array<string>} [itemJson.parentJob] - Array of parent job IDs
   * @param {number} [itemJson.blueprintTypeID] - Blueprint type ID
   * @param {string} [itemJson.groupID] - Group ID for organization
   ...
   * @param {boolean} [itemJson.deleted] - Whether job is deleted
   * @param {number} [itemJson.deletedTimeStamp] - Deletion timestamp
   * @param {boolean} [itemJson.archived] - Whether job is archived
   * @param {boolean} [itemJson.archiveProcessed] - Whether archive is processed
   ...
   *
   * @param {Object} newItem - Extra cost item to add
   * @param {string} newItem.id - Unique identifier for the cost item
   * @param {number} newItem.extraValue - Cost value
   */
  ...
  removeParentJobsNotIncludedInInput(includedJobIDs) {
    ...
    this.parentJob = this.parentJob.filter((id) => parentsToKeep.includes(id));
  }
