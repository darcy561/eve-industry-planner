...
export const JOB_DEPENDENCY_TREE_DIALOG_EVENT = "jobDependencyTreeDialog";

/**
 * Opens the job dependency tree `ContentDialog` (see `JobDependencyTreeDialog.jsx`).
 *
 ...
 */
export function openJobDependencyTreeDialog(payload = {}) {
  const p = payload;
  const toIdList = (x) => {
  ...
  eventEmitter.emit(JOB_DEPENDENCY_TREE_DIALOG_EVENT, {
    isOpen: true,
    openSession:
 ...
 * (related jobs, group pool, focus) is handled inside `JobDependencyTreeDialog`.
 *
 * @param {object} p
...
export function closeJobDependencyTreeDialog() {
  eventEmitter.emit(JOB_DEPENDENCY_TREE_DIALOG_EVENT, { isOpen: false });
}
