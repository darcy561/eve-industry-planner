...
- Capacity soak (`testing/capacity_soak`): prefer host + `DOCKER_HOST` for desired replicas; shorten operator `scale_*` for demos; worker pauses Asynq then enqueues; websocket reuses soaklib hold — details in [harness.md](./harness.md) § Capacity soak.
