# ops / status / msg / process / catalog — tests

Live SoT for test depth under `internal/ops`, `internal/status`, `internal/msg`, `internal/process`, `internal/catalog`. Behaviour → [verbs.md](../../deployment/deployment-tool/cli/verbs.md), [messaging.md](../../deployment/deployment-tool/cli/messaging.md). Module map → [contents.md](./contents.md).

## Entrypoints

```bash
# from deployment-tool/
go test ./internal/ops/ ./internal/status/ ./internal/msg/ ./internal/process/ ./internal/catalog/
```
...
| `catalog` | Service order “prefer” ordering |
...
- Most of `catalog` (`verbs.go`, `services.go` metadata)
