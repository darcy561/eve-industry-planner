# Core rebuild: SeaweedFS + readiness (history)

> **Migration / work log — not live SoT.** Live core behaviour → [backend/core/core.md](../backend/core/core.md).
>
...
- **SeaweedFS lives on Swarm** from day one; **provisioned by deployment-tool** inside existing bring-up (`EnsureS3` / Ready); **no parallel data-* verb family**.
...
- **Networks:** attachable overlay [`eip-core`](../stack/network.md) for app/data DNS; per-consumer stack overlays **`eip-docker-*`** for socket proxies only (Traefik / ws-router / #18).
- **Singletons** stay in scope; Redis lease + workload registry; can trail object-store slices.

Data plane lives on the Swarm **data fragment**; app services on the **app fragment**. Same **order + don't cross-roll** rules. Preferred host UX is the **Deployment Tool** (CLI / TUI).
...
```text
Swarm starts New (standby)
  New: /healthy live; /ready = handoff-ready (NOT holding lease)
Swarm marks New Healthy → SIGTERM Old
  Old: release lease:core:primary; stop scheduler/changestream; persist resume tokens; exit
New acquires lease:core:primary
  New: start scheduler + changestream StartAfter resume
  (/ready stays standby-shaped for the next roll)
```
...
| **Application Mongo indexes / RS / users / preimages** | **Not on core** — owned by deployment-tool `dataplane.EnsureMongo` / `eip ensure-mongo` (`IndexSpecs`, `PreimageCollections` in `deployment-tool/internal/dataplane/mongo`). `eip up`/`dev` Ready gates app deploy. Legacy `mongo-setup.sh` is not the SoT. |
...
## How Mongo is ensured (locked — deployment-tool)
...
| Caller SoT | [`dataplane.EnsureMongo`](../../deployment-tool/internal/dataplane/ensure_mongo.go) → `mongo.Ensure` |
...
Details: [deploy.md](../deployment/deployment-tool/cli/deploy.md), [verbs.md](../deployment/deployment-tool/cli/verbs.md), [variables.md](../deployment/deployment-tool/cli/variables.md).
...
| Bucket | Name **`static-data`** (+ `static-data-test`); ensured by deployment-tool `dataplane.EnsureS3` / `eip ensure-s3` |
...
```text
Data fragment:  mongo · redis · nats · seaweedfs
App fragment:   traefik · api · worker · core · frontend

worker → seaweedfs   (put versioned SDE objects)
api    → seaweedfs   (get + local cache)
api    → traefik     (ready if cache/objects OK)
api/worker → mongo
core does not own the shared static-data volume gate
```
...
### 1. Ensure via deployment-tool — not bootstrap containers
...
- Data-plane day-2 → bring-up order + `eip ensure-*` / Ready
...
| **#22** Data-plane updates | Pattern now with SeaweedFS + ensure/Ready |
| **#3** static volume | Hard cut off `static_data_files` for SDE — objects in SeaweedFS `static-data` ([secrets.md](../stack/secrets.md) § Remaining host binds) |
