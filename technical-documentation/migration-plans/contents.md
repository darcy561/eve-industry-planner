...
| Realtime delivery shape (one internal shape every inbound path converts to, a family-keyed table saying how a family is delivered, one walk replacing six fan-out functions, and the vocabulary growing to cover every type the service sends) | [realtime-delivery-shape/contents.md](./realtime-delivery-shape/contents.md) |
