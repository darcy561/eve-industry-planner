...
- **Code touched:** `services/websocket/server/{dispatch,org_indexes,scope_upgrade,session_resume,reader,handler,types,server}.go`, `services/websocket/server/outgoinglogic/{decode,outgoing}.go`, `services/shared/core/internaljwt/jwt.go`, `services/api/v1endpoints/{authenticate,refresh}.go`, `services/core/changestream/watcher.go`, docs under `technical-documentation/migration-plans/websocket-realtime/`.
- **Links:** (none)
...
- **Collection:** Mongo `user_job_documents` — same [`models.Job`](../../../services/shared/models/job.go) shape; indexes on `(_meta.owner.kind, _meta.owner.id, displayOnPlanner)` and `(_meta.owner.kind, _meta.owner.id, groupID)` (`IndexSpecs` in [`deployment-tool/internal/dataplane/mongo`](../../../deployment-tool/internal/dataplane/mongo/)); `changeStreamPreAndPostImages` via deployment-tool `PreimageCollections` / `dataplane.EnsureMongo` (`eip ensure-mongo`).
...
- **Code / infra touched:** `frontend/src/Realtime/{applyRemoteMessage.js,handlers/**}`; `technical-documentation/migration-plans/websocket-realtime/implementation.md`.
- **Links:** (none)
...
- **Decision:** Treat as a **project rule** for this migration folder: any change to realtime-related code, contracts, security, or ops must update **`implementation.md`** (facts), **`interactions.md`** (dated note for decisions / sessions), and **`plan-todo-tracker.md`** when checklist items move—same PR when practical or an immediate follow-up commit. The rule and this request are duplicated in [readme.md](./overview.md#documentation-maintenance-required) and [implementation.md](./implementation.md#keeping-this-document-accurate-project-rule).
- **Code / infra touched:** Docs only (`README.md`, `implementation.md`, `interactions.md`, `technical-documentation/migration-plans/README.md`).
- **Links:** (none)
...
- **Decision:**
  ...
  - **Subscribe ACL:** `(*Server).docSubscribeAuthorized` — `users` / `application_settings` require doc id == JWT `account_id`; `jobs`, `archivedJobs`, `groups`, `build_stats` require Mongo `DocumentExistsByID` (`_id` + `_meta.owner`); all other collections denied. Enforced in `reader.go` and `processor.go`.
  ...
  - **Docs:** Added [`implementation.md`](./implementation.md); expanded [`README.md`](./overview.md); updated [`technical-documentation/migration-plans/README.md`](../contents.md) status; unit tests `services/websocket/server/subscribe_auth_test.go`.
- **Code / infra touched:** `services/shared/core/internaljwt/**`, `services/api/**` (JWT imports), `services/websocket/server/**`, `frontend/src/Realtime/**`, `frontend/src/Zustand/**`, `frontend/src/App.jsx`, `frontend/src/routes/signout.jsx`, `technical-documentation/migration-plans/**`.
...
- **Code / infra touched:** `technical-documentation/migration-plans/websocket-realtime/plan-snapshot.md`, `scripts/sync-websocket-migration-plan.sh`, `technical-documentation/migration-plans/websocket-realtime/README.md`.
- **Links:** (none)
...
- **Decision:** Use `technical-documentation/migration-plans/websocket-realtime/` with `README.md` (overview + maintenance rules), `interactions.md` (this log), and `plan-todo-tracker.md` (todo checklist).
- **Code / infra touched:** Docs only (`technical-documentation/migration-plans/*`).
- **Links:** (none)
...
- **Docs track code:** Any edit to migration-related code or contracts must update `implementation.md` / `interactions.md` (and tracker or plan snapshot as applicable)—see [readme.md](./overview.md#documentation-maintenance-required) and the dated entries above (including **2026-04-18** account-scoped / JetStream refresh).
