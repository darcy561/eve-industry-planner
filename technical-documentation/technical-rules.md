...
- **Review a `go fix` diff that touches a struct literal.** Its composite-literal folding does not check whether the target field is already set elsewhere in the same literal — a trailing assignment restating a field the literal already sets is folded into a **duplicate field key**, which does not compile. Read that diff by eye rather than applying it unreviewed.
