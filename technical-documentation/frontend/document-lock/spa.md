...
| `resolveDocumentLockApiTarget(collection, docID)` | Maps `account_job_documents` + group member job → `account_job_groups` + `groupID`. Used by `requestAccess`, `handOverEditAccess`, `acceptAccessRequest`, and `forceReleaseSameAccountEditLock` so the server runs group handover + per-job cascade. Header primary registration prefers the **group** scope when both register. |
