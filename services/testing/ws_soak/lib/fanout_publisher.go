...
func buildLiveJob(rng *rand.Rand, snap liveSnapshot, kind fanoutMsgKind, docID, collection string, seq int) (fanoutJob, bool) {
	...
	case fanoutMsgCorpFull:
		...
		return fanoutJob{
			...
			DocID:          docID,
			Collection:     collection,
			TenantString:   wsplacement.TenantKeyCorporation(fmt.Sprintf("%d", corpID)),
			CorporationID:  fmt.Sprintf("%d", corpID),
	...
	case fanoutMsgAllianceFull:
		...
		return fanoutJob{
			...
			DocID:          docID,
			Collection:     collection,
			TenantString:   wsplacement.TenantKeyAlliance(fmt.Sprintf("%d", allID)),
			AllianceID:     fmt.Sprintf("%d", allID),
			ExpectAccounts: append([]string{}, members...),
			Expect:         len(members),
		}, true
	...
	case fanoutMsgAllianceDownCorp:
		...
		return fanoutJob{
			...
			TenantString:        wsplacement.TenantKeyAlliance(fmt.Sprintf("%d", allID)),
			AllianceID:          fmt.Sprintf("%d", allID),
			ScopeCorporationIDs: []string{fmt.Sprintf("%d", corpID)},
			ExpectAccounts:      append([]string{}, members...),
			Expect:              len(members),
		}, true
