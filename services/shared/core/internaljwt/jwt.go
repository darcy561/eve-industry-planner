...
// UnmarshalJSON implements custom JSON unmarshaling for alliance id lists.
...
type InternalClaims struct {
	...
	Alliances     AllianceIDs    `json:"alliances"`    // Alliance IDs for realtime scope ceiling (optional; empty if none)
...
// Pass nil for corporations or alliances when not used.
func GenerateInternalJWT(privateKey *rsa.PrivateKey, characterHash string, kid string, sessionID string, corporations []int64, alliances []int64) (string, *InternalClaims, error) {
	if privateKey == nil {
	...
	if len(corporations) > 0 {
		corpIDs = CorporationIDs(corporations)
	}
	allianceIDs := AllianceIDs{}
	if len(alliances) > 0 {
		allianceIDs = AllianceIDs(alliances)
	}
