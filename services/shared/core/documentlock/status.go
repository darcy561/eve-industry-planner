...
func StatusBatchResults(ctx context.Context, rdb *redis.Client, accountID string, jobDocIDs, groupDocIDs []string) (jobResults map[string]any, groupResults map[string]any, err error) {
	...
	jobResults, err = pipelinedStatusForCollection(ctx, rdb, accountID, eipmongo.CollectionAccountJobDocuments, jobDocIDs)
	if err != nil {
		return nil, nil, err
	}
	groupResults, err = pipelinedStatusForCollection(ctx, rdb, accountID, eipmongo.CollectionAccountJobGroups, groupDocIDs)
	if err != nil {
		return nil, nil, err
	}
