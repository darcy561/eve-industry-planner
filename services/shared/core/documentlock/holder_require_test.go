...
func TestCollectLockHeldElsewhereRejects_emptySessionErrors(t *testing.T) {
	...
	_, err := CollectLockHeldElsewhereRejects(context.Background(), rdb, testOwner, "", eipmongo.CollectionJobDocuments, []string{"j1"}, nil)
	if err == nil {
...
func TestCollectLockHeldElsewhereRejects_noRedis(t *testing.T) {
	t.Parallel()
	got, err := CollectLockHeldElsewhereRejects(context.Background(), nil, testOwner, "sess-a", eipmongo.CollectionJobDocuments, []string{"j1"}, nil)
	if err != nil || got != nil {
...
func TestCollectLockHeldElsewhereRejects_rejectsOtherHolder(t *testing.T) {
	...
	rej, err := CollectLockHeldElsewhereRejects(ctx, rdb, testOwner, "sess-me", eipmongo.CollectionJobDocuments, []string{"j-x", "j-free"}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCollectLockHeldElsewhereRejects_groupHolderBypassesJobLock(t *testing.T) {
	...
	rej, err := CollectLockHeldElsewhereRejects(ctx, rdb, testOwner, "sess-me", eipmongo.CollectionJobDocuments, []string{"j-member"}, bypass)
	if err != nil {
		t.Fatal(err)
	}
	if len(rej) != 0 {
