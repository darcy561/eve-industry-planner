...
func TestCollectLockHeldElsewhereRejects_emptySessionErrors(t *testing.T) {
	...
	_, err := CollectLockHeldElsewhereRejects(context.Background(), rdb, testAccountID, "", mongocore.CollectionUserJobDocuments, []string{"j1"})
	if err == nil {
...
func TestCollectLockHeldElsewhereRejects_noRedis(t *testing.T) {
	t.Parallel()
	got, err := CollectLockHeldElsewhereRejects(context.Background(), nil, testAccountID, "sess-a", mongocore.CollectionUserJobDocuments, []string{"j1"})
	if err != nil || got != nil {
...
func TestCollectLockHeldElsewhereRejects_rejectsOtherHolder(t *testing.T) {
	...
	rej, err := CollectLockHeldElsewhereRejects(ctx, rdb, testAccountID, "sess-me", mongocore.CollectionUserJobDocuments, []string{"j-x", "j-free"})
	if err != nil {
		t.Fatal(err)
	}
