...
func TestCollectLockHeldElsewhereRejects_emptySessionErrors(t *testing.T) {
	...
	_, err := CollectLockHeldElsewhereRejects(context.Background(), rdb, testAccountID, "", mongocore.CollectionUserJobDocuments, []string{"j1"}, nil)
	if err == nil {
...
func TestCollectLockHeldElsewhereRejects_noRedis(t *testing.T) {
	t.Parallel()
	got, err := CollectLockHeldElsewhereRejects(context.Background(), nil, testAccountID, "sess-a", mongocore.CollectionUserJobDocuments, []string{"j1"}, nil)
	if err != nil || got != nil {
...
func TestCollectLockHeldElsewhereRejects_rejectsOtherHolder(t *testing.T) {
	...
	seedLock(t, rdb, testAccountID, mongocore.CollectionUserJobDocuments, "j-x", LockRecord{
		HolderSessionID: "sess-other",
		AccountID:       testAccountID,
		ExpiresAtUnix:   time.Now().Add(time.Minute).Unix(),
	})
	rej, err := CollectLockHeldElsewhereRejects(ctx, rdb, testAccountID, "sess-me", mongocore.CollectionUserJobDocuments, []string{"j-x", "j-free"}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCollectLockHeldElsewhereRejects_groupHolderBypassesJobLock(t *testing.T) {
	...
	seedLock(t, rdb, testAccountID, mongocore.CollectionUserJobDocuments, "j-member", LockRecord{
		HolderSessionID: "sess-other",
		AccountID:       testAccountID,
		ExpiresAtUnix:   time.Now().Add(time.Minute).Unix(),
	})
	...
	rej, err := CollectLockHeldElsewhereRejects(ctx, rdb, testAccountID, "sess-me", mongocore.CollectionUserJobDocuments, []string{"j-member"}, bypass)
	if err != nil {
		t.Fatal(err)
	}
	if len(rej) != 0 {
