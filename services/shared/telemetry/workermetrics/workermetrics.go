...
func metrics() *asynqTaskMetrics {
	initOnce.Do(func() {
		...
		holder = &asynqTaskMetrics{
			...
			durationMs: telemetry.Must(m.Float64Histogram("worker.asynq.task.duration_milliseconds",
				metric.WithUnit("ms"),
				metric.WithDescription("Wall time to execute one Asynq task handler (milliseconds)"),
			)),
			tasksTotal: telemetry.Must(m.Int64Counter("worker.asynq.tasks_total",
