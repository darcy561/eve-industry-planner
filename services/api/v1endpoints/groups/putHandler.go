...
func (h *Handlers) PutGroupsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if h.locks.Redis != nil {
		...
		rejects, lerr := documentlock.CollectLockHeldElsewhereRejects(ctx, h.locks.Redis, accountID, sessionID, eipmongo.CollectionAccountJobGroups, groupIDs, nil)
		if lerr != nil {
	...
	if sessionID != "" && h.locks.Redis != nil && len(result.Deltas) > 0 {
		for _, delta := range result.Deltas {
			...
			held, herr := documentlock.LockHeldBySession(ctx, h.locks.Redis, accountID, eipmongo.CollectionAccountJobGroups, delta.GroupID, sessionID)
			if herr != nil {
