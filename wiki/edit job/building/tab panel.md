...
The Tab Panel manages linking ESI industry jobs for the Building stage. It provides two tabs for viewing available in-game industry jobs that can be linked and managing currently linked jobs with progress tracking.

## Overview

The Tab Panel provides:
- Available Jobs tab showing ESI industry jobs from the API that match this planner job
- Linked Jobs tab displaying currently connected ESI industry jobs with progress and status
- Job linking to connect the planner job to in-game EVE Online industry jobs
...
- Behavior: 
  - Links the in-game job to this planner job
...
#### No Matching Jobs
- Message: "There are no matching industry jobs from the API that match this job." (no matching ESI industry jobs for this planner job)
...
### Empty State
- Message: "You currently have no industry jobs from the ESI linked to this job." (no ESI industry jobs linked yet)
...
- Monitor time remaining for linked ESI industry jobs
