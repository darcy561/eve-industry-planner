...
In Selling, the main action is Archive Job for standalone planner jobs—move finished work off the active board while keeping a copy for cost history and reporting. Group jobs are archived from the group workflow instead; this button appears only when the job is not in a group and you are logged in.
...
### Data Removal
When archiving:
- Active board: Planner job removed from the workflow board
