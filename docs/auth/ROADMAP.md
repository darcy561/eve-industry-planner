...
- [README.md](./README.md) — vocabulary, wire contracts, flows, file index
- [BACKEND.md](./BACKEND.md) — Go API + WS + Redis
- [FRONTEND.md](./FRONTEND.md) — SPA bootstrap, rotate, signout, Tranquility gate
...
- **Docs (#1)** — [README](./README.md), [BACKEND](./BACKEND.md), [FRONTEND](./FRONTEND.md); internal JWT/JWKS removed.
...
**Where:** `services/api/v1endpoints/sso/*`, [README §6.1–6.4](./README.md#6-end-to-end-flows).
...
**Where:** `middleware/auth.go`, [BACKEND §2](./BACKEND.md#2-middleware--authconstructor).
...
**Where:** `websocket/server/handler.go`, `realtimeClient.js`, [FRONTEND §6](./FRONTEND.md#6-realtime-auth-integration).
...
**Env vars (auth-related):** `EVE_CLIENT_ID`, `EVE_CLIENT_SECRET`, `REDIS_*`, `REFRESH_TOKEN_AES_*`, `AUTH_SESSION_CLEANUP_DRY_RUN` — see [README §9](./README.md#9-environment-variables).
...
- Internal JWT / JWKS — removed; [README §2](./README.md#2-what-changed-compared-to-the-previous-system).
- `docs/api-v1-session-esi.md` — may overlap; prefer `docs/auth/*`.
- `firebaseadmin` — migration/admin only, not request auth ([BACKEND §10](./BACKEND.md#10-servicessharedfirebaseadmin)).
