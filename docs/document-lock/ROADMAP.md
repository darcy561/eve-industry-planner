...
- **Quiet solo UI + snackbars** — contention-gated ownership toasts (`useLockVacancySnackbar` + `scopeHasOtherSessionContention`), extend nudge only under lease pressure, mount bootstrap (`lockScopeBootstrapped`); see [FRONTEND.md § Snackbars](./FRONTEND.md#snackbars).
