...
> **Handoff status (2026-08-02):** **Compose runtime retired** — Swarm fragments only (`docker-stack.data.yml` / `docker-stack.yml` / optional `docker-stack.obs.yml`). Stub `docker-compose.yml` is cleanup-only. **Host tool is `eip` only** ([DEPLOYMENT.md](../DEPLOYMENT.md): bootstrap → `eip init` → `eip up`; day-2 `eip secrets` / `sync` / `update` / `rebuild` / `logs` / `cli` / `shutdown`). **Make retired (#17).** **Done:** **#3 / #4 / #5 / #7 / #15 / #16 / #17 / #23 / #24 / #31 / #32 / #33 / #34 / #35**; **#21** minimum + force-close; core boxed (**#9–#14 + #28**). Data ensure = `EnsureS3` ‖ `EnsureMongo` (Ready). **#8** soft divert / hosted-tenant still open. **#19** sync apply landed; #18 schema consume still open. **#23** day-2 ship via **`eip update`** / **`eip rebuild`** (standard Swarm rolls — digest-reconcile / bake; dual-warm waves **withdrawn**). **Next:** **#18** track (**#19 remainder → #30 → #27 → #18**); Ensure `*_API` users follow-up; network polish **#36** (small). Docs: DEPLOYMENT, ENV, [NETWORK.md](./NETWORK.md) (single SoT), [ENGINEERING.md](../admintool/ENGINEERING.md), STACK, ROADMAP.

Tracks the single-host Swarm stack: **data fragment** (mongo/redis/nats/SeaweedFS/Prometheus) + **app fragment** (Traefik, api, websocket, worker, ws-router, core, frontend); optional Swarm **observability** fragment (`docker-stack.obs.yml`, #34). **Operator surface: [`eip`](../DEPLOYMENT.md) / [`admintool/`](../../admintool/)** (TUI + CLI).
...
1. **Goal of next implementation slice:** start **#18** capacity-controller prep (**#19 remainder → #30 → #27 → #18**). Compose→Swarm migrate **done**. **#32/#34** done. Hosted-tenant / soft divert parked with **#18**. **Core boxed off** (#9–#14 + #28). Operator path is **`eip`** — do not add Make targets.

2. **Pickup order:** [Recommended pickup order](#recommended-pickup-order) — next live work is the **#18** track (then **#20**, **#22** last).

3. **WS placement (locked):** Redis **tenant -> websocket slot** via Swarm **`eip_ws_router`**; Traefik routes `/ws` -> router. Cookie `eip_tenant_affinity` is the key. Sticky = fallback. During Swarm rolls, router **prefers newest bake** among eligible slots. Drain + force-close: [WEBSOCKET.md](./WEBSOCKET.md). Ops CLI for placement was retired with Make; next home is **#18**.

4. **Auth** rolls **in parallel** - account-key cookie now; widen when corp/alliance claims exist.

5. **Related roadmaps:** [document-lock](../document-lock/ROADMAP.md) (multi-tenant #30-#37, especially #32), [auth](../auth/ROADMAP.md), [DEPLOYMENT.md](../DEPLOYMENT.md), [ENGINEERING.md](../admintool/ENGINEERING.md), stack YAML in project home / kit.

6. **Code anchors already in tree:** `services/shared/core/instanceid`, `services/core/singleton` + `redis/lease`, websocket JetStream durables, `tenant_affinity_cookie.go`, `services/ws-router/` (`preferNewestSlots`), stack Traefik `/ws` → `eip_ws_router` labels, **`admintool/`** (deploy/sync/secrets/rebuild/update/cli — replaces deleted `scripts/swarm/*`).

7. **Testing:** grow [Testing & simulation](#testing--simulation) / **#25-#29** as you implement; weave tests into PRs for #4/#8/#18/#21/etc. Host CI: `admintool` `go test`; stack smoke via **`eip`** against a local Swarm.

Companion context:

- Current prod path: **eip-bootstrap** → **`eip init`** → **`eip up`** ([DEPLOYMENT.md](../DEPLOYMENT.md))
- Admintool engineering: [ENGINEERING.md](../admintool/ENGINEERING.md) (Ready, Ensure*, sync, secrets, TUI)
- Shared network contract: [NETWORK.md](./NETWORK.md) (`eip-core` mesh + `eip-public` edge + `eip-obs` / `eip-docker-*`)
- Replica identity: [STACK.md](./STACK.md) § Replica identity + `services/shared/core/instanceid` → JetStream durables / OTLP `service.instance.id`
- Elastic Swarm stack (live): [STACK.md](./STACK.md) + `docker-stack.yml` — Traefik + api/websocket/worker/ws-router/**core**/**frontend**
...
- WS placement: [WS_ROUTER.md](./WS_ROUTER.md) — Redis tenant→slot; sticky fallback; prefer-newest bake mid-roll
- App images / day-2 rolls: [ENGINEERING.md](../admintool/ENGINEERING.md) — **`eip update`** (pull+digest-reconcile) / **`eip rebuild`** (bake)
- Secrets / apply: [ENV.md](./ENV.md) — **`.env` = secrets** → **`eip secrets`**; **`eip.config.yaml`** → **`eip sync`**; FE public knobs via `x-frontend-public-env`; **`eip ensure-s3`** / **`eip ensure-mongo`**
- Core control plane: [CORE_REBUILD.md](./CORE_REBUILD.md) — `lease:core:primary` + `servicemanager`; nested singleton leases; changestream resume in `core/primaryhandoff`
...
| [Backlog](#backlog) | Numbered work items **#1–#35** |
...
| Layer | Today | Pain |
|-------|--------|------|
| Deploy | **`eip up`** / **`eip dev`** — two-pass stack deploy + Ready (`EnsureS3` ‖ `EnsureMongo`) via [admintool](../admintool/ENGINEERING.md) | Day-2: **`eip sync`**, **`eip secrets`**, **`eip rebuild`**, **`eip update`** (#32 / #33 / #23) — avoid full bring-up as a sledgehammer |
| Operator UX | Host **`eip`** binary (TUI + CLI); bootstrap installs it | Make retired (#17); do not reintroduce Makefile ops |
...
**Current topology:** Swarm **data** + **app** + optional **obs** fragments on external `eip`. Frontend on Swarm (**#16**) with `x-frontend-public-env`. Data-plane desired state via admintool **`EnsureS3`** / **`EnsureMongo`**. Operator UX is **`eip` only** (Make retired). Obs addon (#34) done — `addons.observability.enabled` merges `docker-stack.obs.yml`. Compose runtime retired (stub `docker-compose.yml` for leftover cleanup only).
...
Controlled **cordon / drain / evacuate / pin overlays** sit on the same Redis map. Instant reassign on connect keeps the balancer correct when a slot dies; #21 is required for **safe scale-in** (do not cold-kill a hot alliance). Placement map + acceptance **done** (#4). **#21 minimum + force-close:** Redis cordon/pin/evacuate + drain PUBLISH / WS bounce (former `make ws-placement-ops` **removed** with Make). Later: verbs live on **#18 controller** (and `eip`); Redis stays SoT.
...
20. **Same `eip` experience on Windows, Linux, and macOS** - one Go binary (TUI + CLI); bootstrap `.sh` / `.ps1` only places the binary (#17 / #32 / #33 / #35). No OS-specific public command set; no Make/Git Bash requirement for operators.
...
22. **Make is retired** - do not reintroduce Makefile / `scripts/swarm` as an operator surface. Host verbs: [DEPLOYMENT.md](../DEPLOYMENT.md) / [ENGINEERING.md](../admintool/ENGINEERING.md).
...
- Pin / document replica env vars ([STACK.md](./STACK.md) § Replica identity) - **done**.
- Inventory bind mounts -> configs/volumes plan (#3) - **done** ([ENV.md](./ENV.md) § Remaining host binds); adminSDK binds removed; Swarm secrets + narrow Go config landed.
...
- Document `.env` → Swarm secrets + day-2 apply (#24) - **done** ([ENV.md](./ENV.md), [DEPLOYMENT.md](../DEPLOYMENT.md)); operator verbs now **`eip secrets`** / **`eip sync`**.
...
**Landed (local / branch `swarm/hard-cutover`):** bring-up via **`eip up` / `eip dev`** (admintool), data+app(+optional obs) Swarm fragments, Traefik ingress (#31), api/websocket/worker + ws-router + core + frontend (#4/#5/#16), affinity cookie, Redis placement path, Desktop localhost publish. Make + Compose runtime retired (#17 / #34).
...
- ~~Observability addon as Swarm fragment (#34)~~ **done** — `docker-stack.obs.yml` + YAML toggle via admintool.
...
| **#17** Operator surface (`eip`) | `eip sync` / `secrets` / `rebuild` / `update`; YAML edit/reload (scale via YAML; auto = #18); Make retired | Ops path (#32 / #33) — **done** |
...
3. **Manual scale** - `eip sync` / Moby `ServiceUpdate` paths covered by integration or documented drills (not Make).
...
When implementing #4, #6-#8, #11-#13, #18-#21, #23: add/extend tests in the same PR when practical. Track harness foundations under **#25-#29** so work isn’t orphaned. Host unit tests: `admintool` + `services/` Go packages.
...
- **size:** S
- **where:** fragment `networks:` + `engine.Ready` / `stack.ExternalNetworks`; [NETWORK.md](./NETWORK.md) (single SoT; former `NETWORK_MAP.md` folded in)
- **why:** Data + app (+ obs) Swarm fragments need shared DNS on one mesh overlay
- **how (landed):** External attachable **`eip-core`**; stack-owned **`eip-public`** / **`eip-docker-*`** / optional **`eip-obs`**. Ready creates external nets from YAML (not a separate Make ensure).
...
- **size:** S
- **where:** `instanceid.Replica`; [STACK.md](./STACK.md) § Replica identity; [`docker-stack.yml`](../../docker-stack.yml); websocket JetStream durables
...
- **size:** L
- **where:** [ENV.md](./ENV.md) (§ Remaining host binds); `docker-stack.yml`; admintool `swarm` secrets + **`eip secrets`**; `services/shared/core/swarmsecret`; `services/shared/core/config`
...
- **status:** **done** (2026-07-19): swarm provider; affinity cookie; Swarm `eip_ws_router` + Traefik `/ws` cutover ([WS_ROUTER.md](./WS_ROUTER.md); replicas **1**, `start-first`); Redis placement + sticky fallback; acceptance proven on local smoke (former `make smoke-ws-placement` retired with Make).
- **size:** L
...
- **size:** M
- **where:** `docker-stack.yml`; [STACK.md](./STACK.md); admintool `stack` / `dockercli.StackDeploy` (two-pass deploy via **`eip up` / `eip dev`**)
- **why:** Need Swarm-honoured `deploy.update_config` and slot templates
- **how:** Extract elastic services; pin images via `APP_VERSION`; wire volumes (`api_data`, `worker_data`); SDE in SeaweedFS (`objectstore`); reserve `capacity_config` volume for `#19`; optional `eip.capacity.*` deploy labels. Deploy expands `.env` via compose-go (`admintool` Expand) and strips top-level `name:` for Swarm.
...
- **size:** S
- **where:** [ENGINEERING.md](../admintool/ENGINEERING.md); pairs with **#23**
...
- **size:** M
- **where:** [WEBSOCKET.md](./WEBSOCKET.md); `admintool/.../yamldefaults` `websocket:`; `services/websocket/server/cordon_drain.go`; Redis drain PUBLISH / cordon (former Make `ws-placement-ops` removed — next home **#18**); Redis `ws:session_handoff:v1`
...
- **size:** S
- **where:** **`eip cli`** → `admintool/internal/ops/core_cli.go`; TUI More → Command / `:`; [ENGINEERING.md](../admintool/ENGINEERING.md); core `tasks` wrapper unchanged
...
- **where:** frontend in app fragment `docker-stack.yml`; `x-frontend-public-env`; [ENV.md](./ENV.md); #23 / [ENGINEERING.md](../admintool/ENGINEERING.md); admintool images bake / **`eip dev`** / **`eip rebuild`**
...
#### #17 - Operator surface (`eip` / DEPLOYMENT.md) — Make retired
- **status:** **done** (2026-07-23; **Make retired 2026-08-02**) — operator surface is **`eip`** only; Makefile + `scripts/{bootstrap,swarm,lib,ops,test}/` deleted.
- **size:** M
- **where:** [`admintool/`](../../admintool/); eip-bootstrap; `scripts/admintool/build-host.*`; [DEPLOYMENT.md](../DEPLOYMENT.md); [ENGINEERING.md](../admintool/ENGINEERING.md)
...
- **how (landed → retired):** Public Make thinned, then replaced by bootstrap + `eip` (up/dev/sync/secrets/rebuild/update/logs/cli/shutdown/repair/init/ensure-*). Scale = YAML + **`eip sync`** (automatic = **#18**). Cross-platform = one Go binary (not Git Bash Make).
- **acceptance:** Docs teach eip-first bring-up vs apply vs rebuild; no Compose-elastic product path; no Make requirement — **met**
...
- **how:** Dedicated **capacity controller** service only (no app replica calls Docker to scale itself). **Docker socket pattern (locked with Traefik/ws-router):** one `tecnativa/docker-socket-proxy` per trust boundary on its **own** overlay (`eip-docker-traefik` / `eip-docker-ws` / `eip-docker-capacity`) — never mount the sock on the controller, never put proxies on `eip-core`, never share docker nets across consumers, and never widen `traefik-docker-proxy` / `ws-docker-proxy` for Apply. Controller proxy is the only one that may enable `POST` (scale/update) after **#27** dry-run + **#30** executor. **Swap model (locked):** lease-gated **hot-swap from day one** - same spirit as core Phase C (`start-first`, single leader via Redis lease; optional warm standby). New task acquires lease before arming mutations; old task releases lease on SIGTERM. Cooldown/hysteresis state may live in Redis so a roll does not forget recent scale decisions.
  ...
  **Reconciliation loop (golden rule):** `Observe -> Evaluate -> Apply -> Wait`. Policy evaluation is **deterministic and side-effect free**; Docker (and later drain/pin side effects) live only behind the cluster/executor boundary (#30). That keeps #18 coherent as one product owner without one undifferentiated blob.
  ...
  7. **Human ops CLI** - Make **`ws-placement-ops` removed** (#17). When #18 is armed, evacuate/cordon/pin verbs live on the **capacity controller** (and `eip`); do not restore a parallel Redis script writer.
...
- **size:** M
- **where:** [`yamldefaults.DefaultConfig`](../../admintool/internal/kit/templates/yamldefaults/default.go); `eip.config.yaml` (project home); admintool `config.Sync` / Expand; [ENV.md](./ENV.md); [ENGINEERING.md](../admintool/ENGINEERING.md)
...
- **how (seeded):** Versioned defaults in kit templates; Go validate; **`eip sync`** / stack expand → **ephemeral** sync-env (not a durable `.eip-sync.env`). Bridges: replicas=`min`, `eip.capacity.*`, Traefik host publish + dashboard/Grafana paths. Concurrency / client cutoff via Moby `ServiceUpdate`, not sync-env.
- **how (still open):** production mount / richer schema for #18 (`scale_timing`, `capacity_controller_managed`, soft targets); dry-run policy print for controller
- **acceptance (partial):** Example YAML in-repo; **`eip sync`** + obs toggle apply without data-plane bounce — **met**. Remaining: #18 consume path.
...
- **how (direction - prefer reconnect over live TCP migrate):**
  ...
  2. **Operations:** evacuate slot / move tenant / rebalance - write overlays, cordon (refuse new upgrades), signal reconnect; clients land via router using updated Redis state. Former Make `ws-placement-ops` **removed**. **When #18 is armed:** evacuate/cordon/pin verbs on the **capacity controller** (and `eip`); controller applies Redis + NATS drain notify — single write path.
...
- **status:** open - **low priority** - Compose→Swarm migrate done; day-2 data-image bumps still later
- **size:** M
...
- **status:** **done** — day-2 ship is **`eip update`** (GHCR pull + digest-reconcile) / **`eip rebuild`** (local bake + rematerialize); Make `release.sh` / dual-warm waves **withdrawn**; absorbs **#6**. Standard Swarm `start-first` / `stop-first` from stack YAML
- **size:** M
- **where:** admintool `update` / `rebuild` / `images.ReconcileLive`; [ENGINEERING.md](../admintool/ENGINEERING.md) (§ App images); pairs with #33 rebuild
- **why:** Full bring-up can bounce the data layer. Day-2 must ship app images without taking mongo/redis/nats offline
- **how (landed):** **`eip update`** — binary / stack YAML / pull live images + digest-reconcile (force-update services whose running digest drifted). **`eip rebuild`** — bake + rematerialize (no Ready). Swarm owns rolling replacement. ws-router **prefers newest bake** mid-roll; FE snackbar does not block WS reconnect
- **how (withdrawn):** Dual-warm `WARM_ORDER` / 2R cohort / look-ahead cordon release script — do not reintroduce
...
- **status:** **done** (2026-07-23; operator verbs **`eip secrets` / `eip sync`** after #17) — mechanics + public docs; DEPLOYMENT / ENV / STACK aligned (`.env` schema = `EnvFields` Go SoT). Optional `*_API` user creation = Ensure follow-up (out of #24).
- **size:** M
- **where:** [ENV.md](./ENV.md); #32; [DEPLOYMENT.md](../DEPLOYMENT.md); `docker-stack.yml`; admintool `swarm` + `config.Sync`; operator config YAML (#19)
...
- **size:** M
- **where:** `services/...` + `admintool/...` Go tests; optional `tests/swarm/`; CI jobs (admintool + services)
- **why:** Need one place for unit/integration + entrypoints to sims so features don’t ship untestable
- **how:** Package layout; shared fixtures (fake metrics, mini Redis/NATS where needed); document how to run against **`eip dev`**; CI runs unit + cheap integration on PR
- **acceptance:** Documented `go test` entrypoints green in CI; README section in this roadmap or `docs/swarm/TESTING.md` pointer linked from Start here
...
- **size:** M
- **where:** `docker-stack.yml` service `traefik` -> `eip_traefik`; [TRAEFIK.md](./TRAEFIK.md); **`eip up` / `eip dev`**
...
- **status:** **done** (2026-07-20; ephemeral sync-env 2026-07-23; operator surface `eip` after #17) — YAML targeted apply + secrets-only path; Swarm `secret` objects under **#3**; public docs under **#24**
- **size:** M
- **where:** admintool `config.Sync` / `swarm` secrets / `Rematerialize`; [ENGINEERING.md](../admintool/ENGINEERING.md); [ENV.md](./ENV.md); [TRAEFIK.md](./TRAEFIK.md)
...
- **status:** done (2026-07-20; FE on Swarm default 2026-07-23) for **dev day-2** — default = Swarm bakeable app roles (incl. frontend), Docker cache, rematerialize; prod GHCR path still with #23 / **`eip update`**
- **size:** M
- **where:** admintool `Rebuild` / `images` bake; [ENGINEERING.md](../admintool/ENGINEERING.md)
- **why:** Local code changes should not require full down/up of mongo/redis/nats
- **how (landed):** **`eip rebuild`** bakes app images to `:bake`, promotes per-role `TAG_*` on digest change, rematerializes stack (no Ready). Opt-in `--no-cache`. Explicit service selection where supported. No advertise
- **how (still open):** Dedicated GHCR/`APP_VERSION` rebuild path for prod day-2 if needed beyond **`eip update`**
- **acceptance:** Cached rebuild of selected/full app fragment without bouncing healthy data plane

#### #34 - Observability addon (optional; default off)
- **status:** **done** (Swarm fragment + admintool toggle) — pairs with #15 / #19
- **size:** M
- **where:** [`docker-stack.obs.yml`](../../docker-stack.obs.yml); `addons.observability.enabled` in [`yamldefaults`](../../admintool/internal/kit/templates/yamldefaults/default.go); admintool `deploy` materialize / rematerialize / recipe; [ENGINEERING.md](../admintool/ENGINEERING.md)
...
- **size:** M
- **where:** [`docker-bake.hcl`](../../docker-bake.hcl); admintool `images` bake; `docker-stack.dev.yml`; [ENGINEERING.md](../admintool/ENGINEERING.md)
...
- **how (landed):** buildx bake group `swarm` → stable `:bake`, then per-role promote to `${APP_VERSION}-<timestamp>` only when digest changes; digests via `docker image inspect`. **`eip dev`** bakes then two-pass deploy; **`eip up`** pulls GHCR (no bake); **`eip rebuild`** bake + rematerialize. No `stack-force-local` (unique tags roll without `--force`).
...
- **status:** **open** (docs SoT landed 2026-08-02 in [NETWORK.md](./NETWORK.md); code gaps below)
- **size:** S
- **where:** `docker-stack*.yml`; optional admintool NetworkConnect; grafana Traefik labels; [NETWORK.md](./NETWORK.md) § Doc vs code gaps
- **why:** NETWORK audit found claims that do not match live setup; keep one SoT honest before #18 adds another proxy island
- **how / open work:**
  1. **Prometheus ↔ `eip-obs`:** docs/obs header previously claimed sync/up dual-homes prometheus onto `eip-obs` — **not implemented**. Today exporters dual-home onto `eip-core` (works). Decide: keep that as SoT **or** implement prometheus→`eip-obs` attach.
  2. **Grafana HTTPS:** obs router is `entrypoints=web` only; add `websecure` (or document intentional HTTP-only) so `/grafana` matches api/frontend/ws.
  3. **`eip-docker-capacity`:** still **#18** (do not invent a parallel proxy net early).
- **acceptance:** NETWORK.md matches YAML + Ready behavior; no phantom `EIP_NETWORK_NAME` / `engine.NetworkName`; chosen prometheus/obs scrape topology documented and implemented the same way; grafana entrypoints decision recorded in TRAEFIK/NETWORK.
...
**Already landed (do not re-open):** Phase 0; Compose→Swarm migrate (data + app + obs fragments); **#3/#4/#5/#7/#9–#17/#23/#24/#28/#31–#35**; **#21** minimum; **#15/#34** obs; **#32** day-2 verbs. Dual-warm release waves withdrawn.

**Next (live):**

1. **#19 remainder → #30 → #27 → #18** — richer controller YAML consume + cluster seam + **dry-run** before armed capacity controller
2. **#8 / #21 deepen** — hosted-tenant surface + soft divert; armed evacuate/cordon verbs on **#18** / `eip` (Make ops CLI retired)
3. **#20** + extend #26/#29 — selective fan-out
4. **#25–#29** testing harness — weave into the above PRs
5. **#22** data-plane image updates — last
6. **Ensure `*_API` DB users** — optional follow-up on #3
7. **#36** network plane polish — prometheus/`eip-obs` decision, grafana websecure; docs SoT already [NETWORK.md](./NETWORK.md)

---

## Follow-ups (detail later)

1. **#4 done** - placement + acceptance ([WS_ROUTER.md](./WS_ROUTER.md)).
2. **Tenant evacuate / rebalance (#21)** - minimum done; deepen with #8 reconnect signal; cordon/drain/pins on same Redis map.
3. **Hard cutover ops polish** - stack live; secrets/config day-2 (#24/#32 done); roll playbook (#6) absorbed into #23 (Swarm digest-reconcile / bake — dual-warm withdrawn).
...
9. **Operator runbooks** - **day-2 ship** ([ENGINEERING.md](../admintool/ENGINEERING.md) / #23); **`eip rebuild`** (#33); **`eip sync` / `eip secrets`** (#24/#32); **`eip cli`** (#14); evacuate (#21 → #18).
...
16. **Testing architecture (#25-#29)** - CI layout; WS load tool; capacity dry-run; chaos drills; `go test` / `eip` harnesses; what runs in PR vs nightly.
17. **Operator config split + `eip sync` / `eip rebuild`** — done (#17/#32/#33).
18. **Companion docs** - ENV.md / STACK.md / DEPLOYMENT.md / ENGINEERING.md / **NETWORK.md** (single SoT; `NETWORK_MAP` removed) aligned for eip-first ops; remaining rename drift / scrape topology → **#36**.
19. **Buildx local bake (#35) done** - Swarm via `docker-bake.hcl` → `:bake` + per-role `${APP_VERSION}-<timestamp>` in `.eip-local-build.env` + `docker-stack.dev.yml` (group `swarm` includes **frontend**); **`eip up`** pull vs **`eip dev`** bake (no force-local).
20. **`.eip-sync.env` bridge — done/closed (2026-07-23)** — durable file retired. Capacity/ports/paths bridges are **ephemeral** at stack expand and **`eip sync`** (admintool Expand / SyncEnv).
21. **Operator-config package cleanup (defer)** — stack discovery + advertise + YAML apply live in **admintool** (`config` / `stack` / deploy). Keep yaml.v3 as the only YAML parser; clarify public vs internal surfaces. Do **not** start a big rename while still reshaping fragments. **Public ops target:** host **`eip` / `eip.exe`** from [`admintool/`](../../admintool/) — TUI + CLI. See [ENGINEERING.md](../admintool/ENGINEERING.md).
...
| 4-6 | **Release model (#23)** - app images (`APP_VERSION`) ship via **standard Swarm rolls**: **`eip update`** (pull + digest-reconcile) / **`eip rebuild`** (bake). Dual-warm / `WARM_ORDER` waves **withdrawn**. Not dual blue/green stacks. OLD SPA may use NEW backends; FE snackbar does not block WS reconnect. |
...
| 15 | **Phase 0 closed (2026-07-19)** - **#1** eip network, **#2** identity, **#5** stack, Traefik swarm **basic**, **#24** day-2 apply path, companion docs, **`eip_tenant_affinity` cookie**, **#3 inventory**. **Hard cutover branch** subsequently landed ws-router + Swarm Traefik (#4/#31); operator surface **`eip`** (#17). |
...
| 16 | **Full testing + simulation track** (#25-#29): unit/integration, WS connection/affinity load sim, capacity-controller **dry-run** before armed Docker mutations, core failover tests, management ops drills. Weave into feature PRs; harnesses runnable via `go test` / **`eip dev`** where possible. |
...
| 19 | **Bring-up vs apply** - `eip up`/`dev` create the world; `eip sync`/`secrets`/`rebuild`/`update` mutate it. Stack deploy is an internal admintool primitive — day-2 operator verb is **`eip sync`** (YAML) / **`eip secrets`** (`.env`). |
| 20 | **Bring-up** — **`eip up` / `eip dev`** (data → Ready/`EnsureS3`‖`EnsureMongo` → app [+ obs if enabled]). Make + Compose runtime retired. Data / app / optional obs = Swarm fragments. See [ENGINEERING.md](../admintool/ENGINEERING.md). |
...
| 31 | **`APP_VERSION` SoT is `.env`** (non-secret). `eip.config` capacity/ports/paths bridges are **ephemeral sync-env** at expand / **`eip sync`** (no durable `.eip-sync.env`). Local Swarm tags are per-role `${APP_VERSION}-<timestamp>` via bake + `.eip-local-build.env` + `docker-stack.dev.yml` (no `stack-force-local`). |
